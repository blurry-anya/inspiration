...
for (let i = 0, index = 1; i < 10; i++) {
  ...
  ballsObjects.push(
    new Ball(
      startPositionX + index,
      startPositionY + index,
      ballWidth,
      ballHeight,
      getRandomDirection() * (index / 3),
      getRandomDirection() * (index / 3),
      ballSpeed,
      i
    )
...
function loop() {
  ...
  if (!stopFlag) {
    ...
    ballsObjects.forEach((ballObject) => {
      if (ballObject.x > borderX || ballObject.x < 0) {
        // bounce at right and left
        ballObject.directionX *= -1;
      }
      if (ballObject.y > borderY || ballObject.y < 0) {
        // bounce at bottom and top
...
ballsSection.addEventListener("click", (event) => {
  if (balls.length < maxBallAmount && started && !ignoreClickElements(event)) {
    ...
    const counterLabel = document.querySelector(".balls-counter-wrapper");
    if (
      counterLabel.classList.contains("limited") &&
